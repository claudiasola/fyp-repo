...
def findOptimalPrize(lower, upper, data):
	competition_results = [];
	competition_number = 1000;
	step = (upper - lower)/competition_number;
	c_index, total_utility = 0, 0;

	for i in frange(lower, upper, step):
		prize = i;
		winner_index = np.nan;
		max_quality, utility = 0, 0;
		...
		competition_results[c_index].append(prize);
		competition_results[c_index].append(winner_index);
		competition_results[c_index].append(utility);

		total_utility += utility;

		c_index += 1;

	expected_utility = total_utility / len(competition_results);
	print "Expected utility of principal:", expected_utility;

	optimal_prize = findMaximizingPrize(data, expected_utility);

	return optimal_prize;

def computeFixedPriceCompetition(lower, upper, data, N):
	print "\n\nFixed Price Competition\n\n"

	optimal_prize = findOptimalPrize(lower, upper, data);
	print "Optimal prize:", optimal_prize;
...
def main():
	...
	#computeFixedPriceCompetition(lower, upper, data, N);
